...
const unblockRequestsStatusTableConfig = {
  cols: [
    "100px",
    "120px",
    "80px",
  ...
  headers: (tr: TrFunction) => [
    ...
    tr(requestStatusTableI18n.unblockType),
    tr(requestStatusTableI18n.unblockAuthority),
    tr(requestStatusTableI18n.unblockingReason),
...
export const UnblockRequestStatusTable = ({
  data,
  onPageNumberChange,
  ...
  const tableHeaders = useMemo(
    () => unblockRequestsStatusTableConfig.headers(tr),
    [tr]
  );
  ...
  return (
    <>
      <Stack customStyle={styles.table}>
        <Table
          cols={unblockRequestsStatusTableConfig.cols}
          ...
          {data.response?.map((request) => (
            <React.Fragment key={request.idRequest}>
              <Tr>
                ...
                <Text text={request.unblockType} />
                <Text text={request.unblockAuthority} />
                <Text text={request.unblockingReason} />
                <Text
                  text={
